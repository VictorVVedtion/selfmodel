...
```bash
CI=true timeout 120 gemini -G "问题" -s
```
...
```bash
# 并行执行
CI=true timeout 300 gemini -G "@.selfmodel/inbox/research/sprint-<N>-query.md 深度调研" -s &
...
# Leader 捕获输出 → sprint-<N>-report.md
```
...
```bash
# 简单问题不需要文件缓冲
CI=true timeout 120 gemini -G "具体问题" -s
```
