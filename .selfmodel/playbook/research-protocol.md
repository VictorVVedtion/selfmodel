...
| 属性 | 值 |
|------|-----|
| Agent | Gemini CLI（模型内置 Google Search tool，`-y` 模式自动批准） |
| 模型 | gemini-3.1-pro-preview |
| 超时 | 300s（标准）/ 600s（深度研究） |
| 产出 | 研究报告（Markdown），不产出代码 |
| 隔离 | 不需要 worktree（只读操作） |

**注意**: Gemini CLI 没有 `-G` flag。Google Search 是模型内置 tool，在 `-y`（yolo）模式下自动调用。
**注意**: `-s` 是 `--sandbox` 不是 streaming。非交互模式使用 `-p`。
**注意**: 不要用 `yes |` 管道 — 会冲突 stdin，`-y` 已经自动批准所有 tool。
...
```bash
CI=true timeout 120 gemini -p "问题" -m gemini-3.1-pro-preview -y
```

### Type B: 技术调研（双通道）

适用：技术选型、库对比、最佳实践
管道：Gemini + context7（并行）
超时：300s

```bash
# Gemini 搜索（后台）
CI=true timeout 300 gemini \
  -p "$(cat /Users/vvedition/Desktop/selfmodel/.selfmodel/inbox/research/sprint-<N>-query.md) 深度调研" \
...
├── Gemini -y          模型内置 Google Search tool 自动调用
...
# 执行研究（输出自动落盘到 report 文件）
CI=true timeout 300 gemini \
  -p "$(cat /Users/vvedition/Desktop/selfmodel/.selfmodel/inbox/research/sprint-<N>-query.md) 基于上述问题进行深度调研，按照以下格式输出：核心结论、详细发现（含方案对比）、推荐、来源URL、置信度、风险" \
  -m gemini-3.1-pro-preview -y \
  2>&1 | tee /Users/vvedition/Desktop/selfmodel/.selfmodel/inbox/research/sprint-<N>-report.md
```
...
```bash
# 简单问题不需要文件缓冲
CI=true timeout 120 gemini -p "具体问题" -m gemini-3.1-pro-preview -y
```
...
Gemini CLI 输出包含启动噪音（YOLO 提示、IDE 错误、重试信息）。Leader 在读取 report 文件时应跳过前 N 行噪音，从实际内容开始。

可选：用 sed 自动清洗：
```bash
sed -i '' '/^YOLO mode\|^\[ERROR\]\|^Attempt\|^Loaded cached\|^Warning: stdin/d' \
  .selfmodel/inbox/research/sprint-<N>-report.md
```
