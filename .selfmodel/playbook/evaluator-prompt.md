...
2.5. **Focus Area**: Review ONLY the files listed in Deliverables. Peripheral changes (config, imports) get a quick glance but should not drive scoring. If the diff includes files unrelated to the Sprint objective, note them but don't penalize.
...
6. Check for AI Slop patterns: excessive comments on obvious code, unnecessary abstraction layers, defensive null chains, template error handling. These reduce Code Quality score. See quality-gates.md AI Slop section for full list.
7. Output valid JSON matching the schema at the end of this file.
